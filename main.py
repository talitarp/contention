...
class Main(KytosNApp):
    ...
    def get_payload(self, data, block_id, action):
        ...
        if action == 'POST' or action == 'GET':
            payload = {"flows": [{"priority": 30000, "cookie": hex(cookie), "match": {"in_port": int(data["interface"]), "dl_vlan": data["match"]["vlan"]}, "actions": []}]}
        ...
        if action == 'DELETE': 
            ...
            payload = {"flows": [{"priority": 30000, "cookie": hex(cookie), "cookie_mask": 0xffffffffffffffff, "match": {"in_port": int(self.stored_blocks["blocks"][block_id]["interface"]), "dl_vlan": self.stored_blocks["blocks"][block_id]["match"]["vlan"]}, "actions": []}]}
